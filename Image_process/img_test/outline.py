...
# img = cv2.imread(os.path.join(address, image_name), 0)
...
# 读取原图
Src = cv2.imread(os.path.join(address, image_name))
print(Src.shape)
# 重新定义图片大小
...
#   cv2.RETR_CCOMP      建立两个等级的轮廓，上面的一层为外边界，里面的一层为内孔的边界。
...
result = cv2.drawContours(Src, contours, -1, (0, 0, 255), 2)
